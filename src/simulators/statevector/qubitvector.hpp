...
public:
  ...
  // check if this register is on the top of array (always true if array is
  ...
  // swap between chunk
  void apply_chunk_swap(const reg_t &qubits, QubitVector<data_t> &chunk,
                        bool write_back = true);
  void apply_chunk_swap(const reg_t &qubits, uint_t remote_chunk_index);
  void apply_chunk_swap(QubitVector<data_t> &chunk, uint_t dest_offset,
  ...
  //-----------------------------------------------------------------------
  // for batched optimization (Implement them if CPU simulator supports
  // multi-shots)
  //-----------------------------------------------------------------------
  virtual bool batched_optimization_supported(void) { return false; }
...
protected:
  ...
  inline uint_t omp_threads_managed() const {
    return (num_qubits_ > omp_threshold_ && omp_threads_ > 1) ? omp_threads_
                                                              : 1;
  }
  ...
  template <typename Lambda>
  std::complex<double> apply_reduction_lambda(Lambda &&func, size_t start,
                                              size_t stop) const;
...
template <typename data_t>
void QubitVector<data_t>::check_matrix(const cvector_t<data_t> &vec,
...
template <typename data_t>
void QubitVector<data_t>::check_vector(const cvector_t<data_t> &vec,
                                       uint_t nqubits) const {
  const size_t DIM = BITS[nqubits];
  const auto SIZE = vec.size();
  if (SIZE != DIM) {
...
void QubitVector<data_t>::check_checkpoint() const {
  if (!checkpoint_) {
    throw std::runtime_error(
        "QubitVector: checkpoint must exist for inner_product() or revert()");
  }
}
...
//------------------------------------------------------------------------------
template <typename data_t>
void QubitVector<data_t>::initialize_component(
  ...
  auto lambda = [&](const indexes_t &inds,
                    const cvector_t<data_t> &_state) -> void {
    const uint_t DIM = 1ULL << qubits.size();
    std::complex<data_t> cache =
        data_[inds[0]]; // the k-th component of non-initialized vector
...
template <typename data_t>
cvector_t<data_t>
QubitVector<data_t>::convert(const cvector_t<double> &v) const {
...
void QubitVector<data_t>::initialize_from_vector(const list_t &vec) {
  if (data_size_ != vec.size()) {
    std::string error =
        "QubitVector::initialize input vector is incorrect length (" +
        std::to_string(data_size_) + "!=" + std::to_string(vec.size()) + ")";
    throw std::runtime_error(error);
  }
...
template <typename data_t>
void QubitVector<data_t>::initialize_from_vector(
    std::vector<std::complex<data_t>> &&vec) {
  if (data_size_ != vec.size()) {
    std::string error =
        "QubitVector::initialize input vector is incorrect length (" +
        std::to_string(data_size_) + "!=" + std::to_string(vec.size()) + ")";
...
template <typename data_t>
void QubitVector<data_t>::move_from_vector(
    AER::Vector<std::complex<data_t>> &&vec) {
  free_mem();
  data_size_ = vec.size();
...
template <typename data_t>
void QubitVector<data_t>::initialize_from_data(const std::complex<data_t> *vec,
                                               const size_t num_states) {
...
template <typename data_t>
template <typename Lambda>
void QubitVector<data_t>::apply_lambda(Lambda &&func, size_t start,
                                       size_t stop) {
...
template <typename Lambda>
std::complex<double>
QubitVector<data_t>::apply_reduction_lambda(Lambda &&func, size_t start,
                                            size_t stop) const {
...
template <typename Lambda>
std::complex<double>
QubitVector<data_t>::apply_reduction_lambda(Lambda &&func) const {
...
// Error checking
  ...
  return QV::apply_reduction_lambda(0, data_size_, omp_threads_managed(), func,
                                    qubits);
}
...
template <typename Lambda, typename list_t, typename param_t>
std::complex<double>
QubitVector<data_t>::apply_reduction_lambda(Lambda &&func, const list_t &qubits,
                                            const param_t &params) const {
    ...
    for (uint_t b = 0; b < blocks; b++)
      for (uint_t i = 0; i < columns; i++)
        for (uint_t j = 0; j < columns; j++) {
          data_[inds[i + b * columns]] +=
              _mat[i + b * columns + DIM * j] * cache[b * columns + j];
        }
  };
...
template <typename data_t>
void QubitVector<data_t>::apply_permutation_matrix(
...
// Error checking
  ...
  case 4: {
    ...
    apply_lambda(lambda,
                 areg_t<4>({{qubits[0], qubits[1], qubits[2], qubits[3]}}));
    return;
  }
  case 5: {
    // Lambda function for permutation matrix
    ...
    apply_lambda(lambda, areg_t<5>({{qubits[0], qubits[1], qubits[2], qubits[3],
                                     qubits[4]}}));
    return;
  }
  case 6: {
    ...
    apply_lambda(lambda, areg_t<6>({{qubits[0], qubits[1], qubits[2], qubits[3],
                                     qubits[4], qubits[5]}}));
    return;
  }
  default: {
...
void QubitVector<data_t>::apply_mcu(const reg_t &qubits,
  ...
  case 2: {
    ...
    auto lambda = [&](const areg_t<4> &inds,
      ...
      data_[inds[pos0]] =
          _mat[0] * data_[inds[pos0]] + _mat[2] * data_[inds[pos1]];
...
template <typename data_t>
void QubitVector<data_t>::apply_rotation(const reg_t &qubits, const Rotation r,
                                         const double theta) {
  switch (r) {
  case Rotation::x:
  ...
  case Rotation::zx:
    ...
    break;
  default:
    throw std::invalid_argument("QubitVector::invalid rotation axis.");
  }
}

template <typename data_t>
void QubitVector<data_t>::apply_chunk_swap(const reg_t &qubits,
                                           QubitVector<data_t> &src,
                                           bool write_back) {
  uint_t q0, q1, t;
...
template <typename data_t>
void QubitVector<data_t>::apply_chunk_swap(QubitVector<data_t> &src,
                                           uint_t dest_offset,
                                           uint_t src_offset, uint_t size) {
...
template <typename data_t>
double QubitVector<data_t>::norm_diagonal(const reg_t &qubits,
                                          const cvector_t<double> &mat) const {
...
//------------------------------------------------------------------------------
template <typename data_t>
double QubitVector<data_t>::norm(const uint_t qubit,
                                 const cvector_t<double> &mat) const {
// Error handling
#ifdef DEBUG
  check_vector(mat, 2);
  ...
  auto lambda = [&](const areg_t<2> &inds, const cvector_t<data_t> &_mat,
                    double &val_re, double &val_im) -> void {
    (void)val_im; // unused
  ...
  return std::real(
      apply_reduction_lambda(lambda, areg_t<1>({{qubit}}), convert(mat)));
}

template <typename data_t>
double QubitVector<data_t>::norm_diagonal(const uint_t qubit,
                                          const cvector_t<double> &mat) const {
// Error handling
#ifdef DEBUG
  check_vector(mat, 1);
...
template <typename data_t>
std::vector<double>
QubitVector<data_t>::probabilities(const reg_t &qubits) const {
...
#pragma omp parallel if (num_qubits_ > omp_threshold_ && omp_threads_ > 1)     \
    num_threads(omp_threads_)
  {
...
//------------------------------------------------------------------------------
template <typename data_t>
reg_t QubitVector<data_t>::sample_measure(
...
inline pauli_mask_data pauli_masks_and_phase(const reg_t &qubits,
...
double QubitVector<data_t>::expval_pauli(const reg_t &qubits,
  ...
  return std::real(
      apply_reduction_lambda(std::move(lambda), (size_t)0, (data_size_ >> 1)));
}
...
double QubitVector<data_t>::expval_pauli(const reg_t &qubits,
  ...
  if (pair_ptr == this->data()) {
    pair_ptr =
        (std::complex<data_t> *)&recv_buffer_[0]; // use receive buffer for pair
  }
  ...
  return std::real(
      apply_reduction_lambda(std::move(lambda), (size_t)0, data_size_));
}
 ...
 ******************************************************************************/
template <typename data_t>
void QubitVector<data_t>::apply_pauli(const reg_t &qubits,
                                      const std::string &pauli,
...
inline std::ostream &operator<<(std::ostream &out,
                                const AER::QV::QubitVector<data_t> &qv) {
